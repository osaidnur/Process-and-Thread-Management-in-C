...
int main()
{
    ...
    // Declare a variable to hold thread attributes for a detached thread
    pthread_attr_t detachedThread;

    // Initialize the thread attributes structure to its default values
    pthread_attr_init(&detachedThread);

    // Set the detach state attribute to create detached threads
    ...
    // Generating the detached Threads  
    for (int i = 0; i < thread_num; i++)
    {
        ...
        *prog = i;
        ...
        // create the detached thread , call the thread function , and pass the argument (i)
        if (pthread_create(th + i, &detachedThread, &threadFunc, prog) != 0)
        {
    ...
    // Destroy the thread attributes structure to release associated resources
    ...
    printf("The execution time for multiple detached threads way = %f second when using %d threads\n", elapsed,thread_num);

    // makes the main thread waits for other threads to complete before terminating
    pthread_exit(0);
}
